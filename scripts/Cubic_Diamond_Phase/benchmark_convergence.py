...
TEMPLATE_PATH = "../../input-templates/Cubic_Diamond_Phase/benchmarking.in"
INPUT_DIR = "../../input-files/Cubic_Diamond_Phase/benchmarking/"
OUTPUT_DIR = "../../data/sample-outputs/Cubic_Diamond_Phase/benchmarking/"
PROCESSED_DATA_DIR = "../../data/processed/Cubic_Diamond_Phase/benchmarking/"
...
def run_ecut_benchmark(  #all the following paramerters are explained in the end
    ...
    for ecut in ecut_list:
        ...
        file_prefix = f"scf_cd_ecut_{ecut}"
        ...
        if success:
            ...
            # Still record the failed run in results so you can trace what went wrong
    ...
    if not successful_df.empty:
        ...
        df["delta_E_Ry_per_atom"] = df["total_energy_Ry"].apply(
            lambda x: (
                ((x - ref_energy) / successful_df["natoms"].iloc[0])
                if pd.notnull(x)
                else None
            )
        )
...
def run_kpoint_benchmark(  #all the following paramerters are explained in the end
    ...
    print("\n==========================================")
    print(" Running K-Point Grid Convergence ")
    print("===========================================\n")

    for k in k_list:
        file_prefix = f"scf_cd_kpoint_{k}"
        in_file = os.path.join(INPUT_DIR, f"{file_prefix}.in")
        out_file = os.path.join(OUTPUT_DIR, f"{file_prefix}.out")
        ...
        if run_qe:
            ...
            print(f"Executing: K-grid = {k}x{k}x{k} (offset 0 0 0)...", end="", flush=True)
            safe_run_qe(cmd,out_file)
...
if __name__ == "__main__":
    ...
    k_values = [4, 6, 8, 10, 12]        # set all the values you'd like to check

    # 'nprocs' is the number of cores you'd like to assign, set it accordingly below.
    
    run_ecut_benchmark(
        ecut_values,
        fixed_k=8,              # set the Kpoint mesh to be taken for all Ecut calculations (offset by 0 0 0)
    ...
    run_kpoint_benchmark(
        k_values,
        fixed_ecut=60,          # set the Ecut value taken for all Kpoint calculations
        dual_ratio=10,          # (ecutrho/ecutwfc) ~ should be 8-12 for USPPs
        template_str=template,  
        nprocs=1,
        run_qe=True             # required for python scripts to run pw.x on linux
